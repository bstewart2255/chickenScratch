...
async function runMigration() {
  ...
  try {
    ...
    if (safetyCheck.rows[0].with_raw_key !== '85') {
    ...
    while (true) {
      ...
      try {
        // Update batch - NO updated_at field
        const updateResult = await client.query(`
          UPDATE shapes 
          SET data_format = 'stroke_data'
        ...
        console.log(`Batch ${batchNumber} completed: ${batchUpdated} records updated (Total: ${totalUpdated})`);
    ...
    // Verify specific migration success
    const verifyMigrated = await client.query(`
      SELECT COUNT(*) as count 
      FROM shapes s
      JOIN backup_phase1_data_format.shapes_backup_20250128 b ON s.id = b.id
      WHERE b.shape_data::jsonb ? 'raw'
        AND s.data_format = 'stroke_data'
    `);
