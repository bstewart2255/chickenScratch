...
async function runPostValidation() {
  try {
    ...
    console.log(`   Status: ${primaryCheck.rows[0].count === '0' ? '✅ PASSED - No base64 formats with raw key found' : '❌ FAILED - Base64 formats with raw key still exist'}`);
    ...
    console.log(`   Status: ${migrationCheck.rows[0].migrated_count === '85' ? '✅ PASSED - All 85 records migrated' : '❌ FAILED - Expected 85, found ' + migrationCheck.rows[0].migrated_count}`);
    ...
    console.log(`   Status: ${imagesCheck.rows[0].image_count === '30' ? '✅ PASSED - All 30 base64 images preserved' : '❌ WARNING - Expected 30, found ' + imagesCheck.rows[0].image_count}`);
    ...
    console.log(`   Status: ${logSummary.rows[0].failed_operations === '0' ? '✅ PASSED - No failures logged' : '❌ FAILED - Errors found in log'}`);
    ...
    console.log(`Migration Status: ${summary.rows[0].remaining_base64_with_raw === '0' && summary.rows[0].migrated_count === summary.rows[0].expected_count ? '✅ SUCCESS' : '❌ FAILED'}`);
